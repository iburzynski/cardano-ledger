...
depositInvariant SourceSignalTarget {source = mockChainSt} =
  let LedgerState {lsUTxOState = utxost, lsCertState = CertState dstate pstate _vstate} = (esLState . nesEs . mcsNes) mockChainSt
      -- TODO handle VState
...
rewardDepositDomainInvariant SourceSignalTarget {source = mockChainSt} =
  let LedgerState {lsCertState = CertState dstate _ _} = (esLState . nesEs . mcsNes) mockChainSt
      -- TODO VState
