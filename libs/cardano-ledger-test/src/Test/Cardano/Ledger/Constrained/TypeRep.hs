...
instance Shaped (Rep era) any where
  ...
  shape MultiAssetR = Nullary 39
  shape PolicyIDR = Nullary 40
  shape (WitnessesFieldR p) = Nary 41 [shape p]
  shape AssetNameR = Nullary 42
  shape DCertR = Nullary 43
  shape RewardAcntR = Nullary 44
  shape ValidityIntervalR = Nullary 45
