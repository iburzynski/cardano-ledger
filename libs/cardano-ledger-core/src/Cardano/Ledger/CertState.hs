...
instance Era era => EncCBOR (CertState era) where
  encCBOR CertState {certPState, certDState, certVState} =
    encodeListLen 3
      <> encCBOR certPState -- We get better sharing when encoding pstate before dstate
      <> encCBOR certDState
      <> encCBOR certVState
...
obligationCertState (CertState DState {dsUnified = umap} PState {psDeposits = stakePools} VState {}) =
