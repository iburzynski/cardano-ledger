...
genValidTriples = resize 50 $ do
  nCreds <- chooseInt (1, 50)
  ...
  ptrs :: [Ptr] <- vectorOf nCreds arbitrary
  pure
...
genExcluding :: (Eq a, Arbitrary a) => [a] -> Gen a
genExcluding as = do
  a <- arbitrary
  if a `elem` as
    then genExcluding as
    else pure a

genInvariant :: Gen (Credential 'Staking StandardCrypto, Ptr, UMap StandardCrypto)
genInvariant = do
  umap@(UMap tripmap ptrmap) <- genValidUMap
  let creds = Map.keys tripmap
      ptrs = Map.keys ptrmap
  cred <-
    oneof
      [ elements creds
      , genExcluding creds
