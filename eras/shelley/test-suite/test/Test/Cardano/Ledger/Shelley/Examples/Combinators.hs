...
feesAndDeposits ppEx newFees stakes pools cs = cs {chainNes = nes'}
  where
    ...
    es = nesEs nes
    ls = esLState es
    CertState dstate pstate vstate = lsCertState ls
...
feesAndKeyRefund newFees key cs = cs {chainNes = nes'}
  where
    ...
    es = nesEs nes
    ls = esLState es
    CertState dstate pstate _vstate = lsCertState ls
    ...
    es' = es {esLState = ls'}
    nes' = nes {nesEs = es'}
    dpstate' = CertState dstate {dsUnified = UM.adjust zeroD key (RewardDeposits (dsUnified dstate))} pstate def
